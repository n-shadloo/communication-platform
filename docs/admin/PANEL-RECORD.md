...
| **App names in the breadcrumb** | "Accounts", "Devices", "Attachments", "Voice rooms" — the words the sidebar uses, on the breadcrumb of every page of each app | 1 to 3. Flip: none | The breadcrumb separator is a `›` that does not mirror; a Persian panel needs the RTL stylesheet before it reads correctly | None — one attribute for each app, and no migration. Revisit: a new app registers a model | Django 6.0.7 — `AppConfig.verbose_name`, read by the admin for the app link in the breadcrumb | Django titles the app label when an `AppConfig` declares none, so `voicerooms` rendered as `Voicerooms`. Declared on all four apps and pinned by `test_every_registered_project_app_names_itself_in_operator_words`, which fails on a derived name rather than on a list of expected ones. Verified on screen 2026-09-04. Current |
