...
| Pinned development distributions | 17 | `grep -cE '^[a-zA-Z0-9._-]+==' requirements/dev.txt` | 2026-09-04 |
...
| Tracked Python files | 226 | `git ls-files '*.py' \| wc -l` | 2026-09-04 |
| Test files | 109 | `git ls-files '*/test_*.py' \| wc -l` | 2026-09-04 |
| Tests collected | 2571, plus 79 subtests. The rise over 1293 is run 12: `api/` and `config/` had no tests at all, seven `core` modules had no test file, and the run added the classes the suite had none of — property-based, malformed input, concurrency, replay and migration | `pytest -q` | 2026-09-04 |
...
|---|---|---|---|
| Full test suite | 109.9 s, 2571 passed, 79 subtests | Developer machine, `pytest -q`, native PostgreSQL 16 and Redis 7 on loopback, random order (`pytest-randomly` seed reported per run). The rise from 49.1 s is run 12: roughly twice the tests, and the property suites run 200 examples each | 2026-09-04 |
| Full test suite, second order | 108.3 s, 2571 passed, 79 subtests | Same machine, a different `pytest-randomly` order in the same session. The spread between the two orders is the cost of the `transaction=True` tests, whose table truncation lands in a different place each run | 2026-09-04 |
| Branch coverage | 98.7 percent — 3100 statements, 32 missed; 478 branches, 14 partial. 77 files are at complete coverage. Every module in run 12's scope (`core`, `api`, `accounts`, `devices`, `vault`, `config`, the migrations) is at 100 percent but for one provably unreachable branch, `core/panel.py` 47->exit, where `storage_label`'s loop cannot exit normally because its last arm always returns. The remainder is in the apps run 13 covers: `realtime/bus.py` at 90.0, `voicerooms/presence.py` at 84.2, `realtime/gateway.py` at 96.7, `attachments/` at 96.5 and 97.4, `ops/audit/log_silence.py` at 97.9, and the prune command at 99.0. Was 96.4 before the run | `pytest -q --cov --cov-branch`, configured in `backend/pyproject.toml`; `migrations/`, `tests/`, `manage.py` and `devices/vectors/generate.py` are excluded per run 12's decision 4, and `.venv/` and `vendor/` because they are not project code. No gate enforces this yet — [0013](decisions/0013-pytest-and-ruff-as-the-test-and-lint-stack.md) sets one in run 13 | 2026-09-04 |
| Gateway suite against a real Redis bus | 8.4 s, 70 passed | Developer machine, `pytest realtime/ -q -p no:randomly`. Every socket test drives the composed ASGI application on the test's own event loop and fans out through Redis publish and subscribe; three of them run a real uvicorn on an ephemeral port | 2026-09-04 |
...
| Offline install of the whole pinned set | 47 hash-verified wheels collected and installed with `--no-index`: 30 production distributions and 17 development ones, one wheel each. **This row read 54 until run 12 and was wrong** — 54 was measured before Channels, daphne and the sixteen packages only they pulled in left `prod.txt`, and the row was re-stamped without being re-measured. At the count it replaced it would have been 43 | `pip download --require-hashes --only-binary=:all: -r requirements/dev.txt`, then `pip install --no-index --find-links` into a fresh venv, on the developer machine | 2026-09-04 |
