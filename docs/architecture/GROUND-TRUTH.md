...
database. Every entry here was measured on the developer machine, against the
repository at the commit the row's date belongs to — `32617e7` for the rows of
phase 4, `e987f28` for the rows of phase 5 run 14. A row that names the VPS states
what the committed operator configuration sets, not what a running host reports,
and says so. The first production measurement replaces it.
...
| coturn | VPS, UDP and TCP 3478 and nothing else — no TLS listener and no certificate — relay ports 50000–51000, `use-auth-secret`, `no-stun`, `user-quota` 20 and `total-quota` 500, every private, carrier-grade-NAT, link-local, loopback and unique-local range denied as a peer, and `log-file=/dev/null` with `no-stdout-log` and `simple-log`. The only media service on the host ([0021](decisions/0021-relayed-webrtc-mesh-and-no-server-room.md)); nothing in the backend reads `TURN_STATIC_AUTH_SECRET` yet, so the relay is deployed and inert until phase 7 | the participants' media, once phase 7 issues a credential | 2026-09-05 (configured, `backend/ops/coturn/turnserver.conf`; pinned by `core/tests/test_settings_posture.py::CoturnPostureTests`) |
...
| Tracked Python files | 254. Phase 6 run 01 added and removed none: the web removal deleted code inside files that stay, not files | `git ls-files '*.py' \| wc -l`; the merge-base figure from `git ls-tree -r --name-only origin/main \| grep -c '\.py$'` | 2026-09-05 |
| Test files | 134, unchanged by phase 6 run 01 | `git ls-files '*/test_*.py' \| wc -l` | 2026-09-05 |
| Tests collected | 3557, plus 79 subtests. 1293 before run 12, 2571 after it, 3558 after run 13, 3575 after run 14; phase 6 run 01 removed 18 net — the tests of the bare-connect handshake, the `auth` frame, the authentication deadline, the Origin allowlist and its deploy check, and the two `DEBUG`-gated documentation tests, against the refusal, no-documentation-route and no-browser-header tests that replaced them | `pytest --collect-only -q` for the count, `pytest -q` for the subtests | 2026-09-05 |
| The OpenAPI document | 32 operations over 27 paths, 52 components, 121 308 bytes at `backend/openapi.json`. The rise over 121 234 is the `maximum` on the two `version` fields run 12 bounded; the rise over 120 716 before it was the `format` of every id and `_date` the document publishes, which changed no response byte | `python manage.py openapi`, then `wc -c` | 2026-09-04 |
...
|---|---|---|---|
| Full test suite | 189.6 s, 3575 passed, 79 subtests | Developer machine, `pytest -q`, native PostgreSQL 16 and Redis 7 on loopback, random order (`pytest-randomly` seed reported per run). 49.1 s before run 12 and 109.9 s after it; the rise to 184.8 s was run 13, which added roughly a thousand tests, most of them driving the whole ASGI stack, and run 14's seventeen cost the rest | 2026-09-05 |
| Full test suite, second order | 187.8 s, 3575 passed, 79 subtests | Same machine, a different `pytest-randomly` order in the same session. The spread between the two orders is the cost of the `transaction=True` tests, whose table truncation lands in a different place each run; it was 9.6 s across the two orders of run 13 and 1.8 s here | 2026-09-05 |
| Branch coverage | 99.97 percent — 3153 statements, none missed; 494 branches, 1 partial. 85 of the 86 measured files are at complete coverage. The single partial branch is `core/panel.py` 47->exit, which is provably unreachable: `storage_label`'s loop cannot exit normally because its last arm always returns. Was 96.4 before run 12 and 98.7 after it; run 13 closed the rest, and run 14's `ops/sbom.py` landed at 100 percent with the rest of `ops` | `pytest -q --cov --cov-branch --cov-fail-under=95`, configured in `backend/pyproject.toml`; `migrations/`, `tests/`, `manage.py` and `devices/vectors/generate.py` are excluded per run 12's decision 4, and `.venv/` and `vendor/` because they are not project code. The floor is enforced from run 13, in `backend/pytest.ini` and in the CI test job ([0013](decisions/0013-pytest-and-ruff-as-the-test-and-lint-stack.md)) | 2026-09-05 |
...
| Offline install of the whole pinned set | 52 hash-verified wheels collected and installed with `--no-index`: 30 production distributions and 22 development ones, one wheel each, none of them a source distribution now that `ops/vendor.sh` carries `--only-binary=:all:`. The whole suite — 3575 tests — runs green inside the venv built from that set. It read 47 before run 13 added jsonschema and its four dependencies, and the rebuilt tree imports. **This row read 54 until run 12 and was wrong** — 54 was measured before Channels, daphne and the sixteen packages only they pulled in left `prod.txt`, and the row was re-stamped without being re-measured. At the count it replaced it would have been 43 | `bash ops/vendor.sh && bash ops/audit/offline_rehearsal.sh` on the developer machine, which prints `PASS: offline rebuild + full suite green.` | 2026-09-05 |
...
| Volatile data never touches disk: presence, signals, rate counters and lockout state | Redis runs with `save ""` and `appendonly no` | A seized disk yields the traffic pattern | 2026-09-03 |
