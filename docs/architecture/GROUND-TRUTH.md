...
| Tracked Python files | 254, of which 2 arrived in this run (`ops/sbom.py` and its test). **The row read 226 and was wrong**: the tree at the phase-4 merge already held 252, so the figure was carried forward rather than re-measured, the same failure the offline-install row below records | `git ls-files '*.py' \| wc -l`; the merge-base figure from `git ls-tree -r --name-only origin/main \| grep -c '\.py$'` | 2026-09-05 |
| Test files | 134, one of them new here. **The row read 109 and was wrong** for the same reason: the phase-4 merge held 133 | `git ls-files '*/test_*.py' \| wc -l` | 2026-09-05 |
| Tests collected | 3575, plus 79 subtests. 1293 before run 12, 2571 after it, 3558 after run 13; run 14 added 17 — seven over the edge and the stop windows, ten over the SBOM generator. Run 12 covered `core`, `api`, `accounts`, `devices`, `vault` and the migrations; run 13 covered `messaging`, `attachments`, `voicerooms`, `realtime`, `ops` and the contract, and added the WebSocket-lifecycle, failure-injection, timeout and schema-driven contract classes the suite still had none of | `pytest -q` | 2026-09-05 |
