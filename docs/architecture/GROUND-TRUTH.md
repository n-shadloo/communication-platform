...
| Tracked Python files | 191 | `git ls-files '*.py' \| wc -l` | 2026-09-04 |
| Test files | 76 | `git ls-files '*/test_*.py' \| wc -l` | 2026-09-04 |
| Tests collected | 1293, plus 48 subtests. The rise over 1226 is the phase-4 reviews: the two schema-format conventions parameterised over every field name the document carries, the retry paragraph of every route rather than of every mutating one, the `405` and `500` envelopes read off the wire, the panel's login driven through the middleware in front of it, the bind that must not queue behind a send's row lock, `access_log off` in every nginx server block, and the app name each registered app declares | `pytest -q` | 2026-09-04 |
| The OpenAPI document | 32 operations over 27 paths, 52 components, 121 234 bytes at `backend/openapi.json`. The rise over 120 716 is the `format` of every id and `_date` the document publishes, which changed no response byte | `python manage.py openapi`, then `wc -c` | 2026-09-04 |
...
|---|---|---|---|
| Full test suite | 49.1 s, 1293 passed, 48 subtests | Developer machine, `pytest -q`, native PostgreSQL 16 and Redis 7 on loopback, random order (`pytest-randomly` seed reported per run) | 2026-09-04 |
| Full test suite, second order | 49.7 s, 1293 passed, 48 subtests | Same machine, a different `pytest-randomly` order in the same session. The spread between the two orders is the cost of the `transaction=True` tests, whose table truncation lands in a different place each run | 2026-09-04 |
