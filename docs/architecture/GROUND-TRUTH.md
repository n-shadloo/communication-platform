...
|---|---|---|---|
| Full test suite | 184.8 s, 3558 passed, 79 subtests | Developer machine, `pytest -q`, native PostgreSQL 16 and Redis 7 on loopback, random order (`pytest-randomly` seed reported per run). 49.1 s before run 12 and 109.9 s after it; the rise to 184.8 s is run 13, which added roughly a thousand tests, most of them driving the whole ASGI stack | 2026-09-04 |
| Full test suite, second order | 175.2 s, 3558 passed, 79 subtests | Same machine, a different `pytest-randomly` order in the same session. The spread between the two orders is the cost of the `transaction=True` tests, whose table truncation lands in a different place each run | 2026-09-04 |
| Branch coverage | 99.97 percent — 3112 statements, none missed; 482 branches, 1 partial. 84 of the 85 measured files are at complete coverage. The single partial branch is `core/panel.py` 47->exit, which is provably unreachable: `storage_label`'s loop cannot exit normally because its last arm always returns. Was 96.4 before run 12 and 98.7 after it; run 13 closed the rest — every module of `messaging`, `attachments`, `voicerooms`, `realtime` and `ops` is now at 100 percent | `pytest -q --cov --cov-branch --cov-fail-under=95`, configured in `backend/pyproject.toml`; `migrations/`, `tests/`, `manage.py` and `devices/vectors/generate.py` are excluded per run 12's decision 4, and `.venv/` and `vendor/` because they are not project code. The floor is enforced from run 13, in `backend/pytest.ini` and in the CI test job ([0013](decisions/0013-pytest-and-ruff-as-the-test-and-lint-stack.md)) | 2026-09-04 |
...
| Offline install of the whole pinned set | 52 hash-verified wheels collected and installed with `--no-index`: 30 production distributions and 22 development ones, one wheel each. It read 47 before run 13 added jsonschema and its four dependencies, and the rebuilt tree imports. **This row read 54 until run 12 and was wrong** — 54 was measured before Channels, daphne and the sixteen packages only they pulled in left `prod.txt`, and the row was re-stamped without being re-measured. At the count it replaced it would have been 43 | `pip download --require-hashes --only-binary=:all: -r requirements/dev.txt`, then `pip install --no-index --find-links` into a fresh venv, on the developer machine | 2026-09-04 |
