...
| Tests collected | 2570, plus 79 subtests. The rise over 1293 is run 12: `api/` and `config/` had no tests at all, seven `core` modules had no test file, and the run added the classes the suite had none of — property-based, malformed input, concurrency, replay and migration | `pytest -q` | 2026-09-04 |
...
|---|---|---|---|
| Full test suite | 106.7 s, 2570 passed, 79 subtests | Developer machine, `pytest -q`, native PostgreSQL 16 and Redis 7 on loopback, random order (`pytest-randomly` seed reported per run). The rise from 49.1 s is run 12: roughly twice the tests, and the property suites run 200 examples each | 2026-09-04 |
| Full test suite, second order | 108.7 s, 2570 passed, 79 subtests | Same machine, a different `pytest-randomly` order in the same session. The spread between the two orders is the cost of the `transaction=True` tests, whose table truncation lands in a different place each run | 2026-09-04 |
