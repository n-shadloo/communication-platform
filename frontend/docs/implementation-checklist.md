...
| WebSocket live delivery | Ready | Piece 06 authenticated gateway/close-code mapping and piece 12 lifecycle supervisor are implemented and unit-tested, and socket envelopes are treated as wake-up hints that always trigger an authoritative REST drain. **Not composed.** Corrected 2026-08-21 under ADR-046: `DioWebSocketGateway`, `GatewayRealtimeSyncAdapter`, `SyncLifecycleSupervisor` and `NetworkingFoundation` are constructed only in tests, and `durableSyncEngineProvider` is read by nothing, so the running artifact opens no socket and runs no drain. Composing this path is ADR-046 follow-up step 1 |
...
| Android normal resume/drain | Durable queue supports it | Piece 12 lifecycle/network resume, durable reconnect, socket-hint, and authoritative drain flow are implemented and unit-tested but **not composed into the application** (ADR-046, 2026-08-21). The same gap stops outbound traffic: `SendConversationEvents` ends at `fanout.prepareAndQueue`, and nothing transmits the durable outbox rows it writes |
| Android background polling | Seven-day durable queue supports it | Piece 12 app-owned best-effort/headless scheduler port complete, with no Firebase. ADR-046 keeps it as the mandatory *eventual* floor and adds an opt-in `specialUse` foreground service above it for *near-real-time, best-effort* delivery, superseding ADR-029's "no persistent service". `dataSync` and `remoteMessaging` remain forbidden and the architecture test that enforces that is unchanged. Concrete WorkManager registration, the durable single-owner delivery lease, the service, the boot receiver, and the Samsung/Xiaomi/AOSP device matrix are all pending |
| Android local notifications | No foreign push by design | Architecture decided (ADR-046), implementation pending. A notification is a projection of committed local state emitted only from the existing `PostInboxCommitWorkPort`, deduplicated by a durable `notified_at`, recovered by query rather than replay, bounded by a grouped summary, and never produced from a transport event. `POST_NOTIFICATIONS` is requested at point of use and refusal is a stated outcome. No notification dependency is declared in `pubspec.yaml` yet and the dependency choice is ADR-046 follow-up step 8 |
