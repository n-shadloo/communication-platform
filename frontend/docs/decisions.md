...
| ADR-009 | Accepted | X3DH + Double Ratchet for device-pair channels | Matches the backend prekey model and asynchronous DM requirements. |
...
| ADR-014 | Accepted | Explicit Android foreground connection mode | Best available self-hosted delivery when users require background reachability. |
...
| ADR-022 | Accepted | Recovery restores history separately from current secure membership | Prevents the UI from claiming that an archive secret recreates ratchets or MLS authorization. |
