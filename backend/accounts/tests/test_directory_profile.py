...
class TestReadProfile:
    def test_returns_the_stored_blob_and_version(self, api, active_user, headers):
        ...
        response = api.get(reverse("user-profile", args=[active_user.id]), **headers)
...
class TestWriteProfile:
    ...
    def test_version_must_increase(self, api, active_user, headers):
        ...
        api.put(
            url,
            {"blob": blob_of(PROFILE_BUCKETS[0]), "version": 5},
            format="json",
            **headers,
        )
    ...
    def test_a_higher_version_overwrites(self, api, active_user, headers):
        ...
        response = api.put(
            url,
            {"blob": blob_of(PROFILE_BUCKETS[1], fill=b"\x09"), "version": 2},
            format="json",
            **headers,
        )
    ...
    def test_a_write_reads_the_row_once(
        self, api, active_user, headers, django_assert_num_queries
    ):
        ...
        with django_assert_num_queries(6):
            response = api.put(
                url,
                {"blob": blob_of(PROFILE_BUCKETS[0]), "version": 2},
                format="json",
                **headers,
            )
