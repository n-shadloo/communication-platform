...
class TestRegister:
    def test_creates_an_inactive_account(self, api, register_url):
        response = api.post(
            register_url, {"username": "bob", "password": GOOD_PASSWORD}, format="json"
        )
    ...
    def test_username_must_match_the_model_validator(self, api, register_url, username):
        response = api.post(
            register_url, {"username": username, "password": GOOD_PASSWORD}, format="json"
        )
    ...
    def test_password_must_pass_django_validators(self, api, register_url, password):
        response = api.post(
            register_url, {"username": "bob", "password": password}, format="json"
        )
    ...
    def test_error_never_echoes_the_submitted_password(self, api, register_url, password):
        response = api.post(
            register_url, {"username": "bob", "password": password}, format="json"
        )
...
class TestLogin:
    def test_unknown_username_still_pays_for_a_dummy_argon2_verify(self, api, login_url):
        with mock.patch("accounts.views.check_password", wraps=check_password) as verify:
            response = api.post(
                login_url, {"username": "ghost", "password": GOOD_PASSWORD}, format="json"
            )
    ...
    def test_inactive_account_is_told_to_wait(self, api, login_url):
        ...
        response = api.post(
            login_url, {"username": "bob", "password": GOOD_PASSWORD}, format="json"
        )
    ...
    def test_without_a_device_only_a_register_scope_token_is_issued(
        ...
        response = api.post(
            login_url, {"username": "alice", "password": PASSWORD}, format="json"
        )
...
class TestLogout:
    ...
    def test_cannot_blacklist_another_accounts_refresh_token(
        ...
        still_valid = api.post(
            reverse("refresh"), {"refresh": victim_refresh}, format="json"
        )
