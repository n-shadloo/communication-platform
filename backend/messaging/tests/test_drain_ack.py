...
def enqueue(device, count, start=1):
    return [QueuedEnvelope.objects.create(recipient_device=device, seq=start + i,
                                          blob=bytes([97 + i]) * SMALLEST_BUCKET)
            for i in range(count)]


@pytest.mark.django_db
def test_a_device_drains_its_own_mailbox_in_seq_order(api, active_user, device,
                                                      auth_headers):
    enqueue(device, 3)
...
@pytest.mark.django_db
def test_a_device_never_sees_another_devices_mailbox(api, active_user, device,
...
def test_ack_deletes_only_the_named_rows(api, active_user, device, auth_headers):
    ...
    resp = api.post(ACK_URL, {"ids": [str(rows[0].id), str(rows[2].id)]},
...
@pytest.mark.django_db
def test_device_a_cannot_ack_device_bs_envelopes(api, active_user, device, auth_headers,
                                                 bob_devices):
    ...
    resp = api.post(ACK_URL, {"ids": [str(r.id) for r in victim_rows]},
                    format="json", **auth_headers(active_user, device))

    assert resp.json() == {"deleted": 0}
    assert QueuedEnvelope.objects.filter(recipient_device_id=bob_devices[0].id).count() == 2
...
def test_a_sibling_device_cannot_ack_its_twins_envelopes(api, active_user, device,
    ...
    resp = api.post(ACK_URL, {"ids": [str(rows[0].id)]},
                    format="json", **auth_headers(active_user, device))
...
@pytest.mark.django_db
@pytest.mark.parametrize("ids", [["abc"], [123], [None], [{}], ["../../etc/passwd"],
                                [str(uuid.uuid4()), "not-a-uuid"]])
def test_malformed_ack_ids_are_a_400_not_a_500(api, active_user, device, auth_headers, ids):
    """Unparsed, these reach a uuid column and raise Django's ValidationError, which DRF
    does not handle."""
    resp = api.post(ACK_URL, {"ids": ids}, format="json",
...
def test_too_many_ack_ids_is_a_400(api, active_user, device, auth_headers):
    resp = api.post(ACK_URL, {"ids": [str(uuid.uuid4()) for _ in range(201)]},
                    format="json", **auth_headers(active_user, device))
...
def test_a_non_object_ack_body_is_a_400_not_a_500(api, active_user, device, auth_headers):
    ...
    resp = api.post(ACK_URL, ["not", "an", "object"], format="json",
                    **auth_headers(active_user, device))
...
def test_a_register_scope_token_has_no_mailbox(api, active_user, auth_headers, method, url):
...
@pytest.mark.django_db
def test_the_drain_response_carries_no_recipient_or_sender_field(api, active_user, device,
                                                                auth_headers):
    enqueue(device, 1)
