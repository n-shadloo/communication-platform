...
class PerDeviceSeqConcurrencyTests(TransactionTestCase):
    ...
    def test_concurrent_sends_to_one_device_never_collide_or_gap(self):
        ...
        def send():
            try:
                ...
                resp = APIClient().post(
                    ...
                    {
                        "messages": [
                            {"device_id": str(self.target.id), "blob": envelope_blob()}
                        ]
                    },
                    format="json",
                    **self.headers,
        ...
        seqs = sorted(
            QueuedEnvelope.objects.filter(recipient_device_id=self.target.id).values_list(
                "seq", flat=True
            )
        )
    ...
    def test_the_unique_constraint_would_catch_a_duplicate_seq(self):
        ...
        QueuedEnvelope.objects.create(
            recipient_device=self.target, seq=1, blob=b"a" * 1024
        )

        with self.assertRaises(Exception):
            QueuedEnvelope.objects.create(
                recipient_device=self.target, seq=1, blob=b"b" * 1024
            )
