...
def test_a_new_device_restores_the_key_backup_byte_identical(api, active_user, device,
                                                             auth_headers):
    ...
    assert api.get(KEYBACKUP_URL, **device2).json() == {"blob": backup_payload,
                                                        "version": 1}


def test_backups_are_scoped_to_their_owner(api, active_user, device, auth_headers,
                                           bob, bob_device):
