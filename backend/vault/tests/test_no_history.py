...
def test_every_history_route_is_gone(api, active_user, device, auth_headers, method,
                                     url):
...
def test_no_registered_model_stores_message_content_past_the_queue():
    ...
    allowed = {"accounts.ProfileBlob.blob", "devices.Device.label_blob",
...
def test_the_key_backup_still_works_end_to_end(api, active_user, device, auth_headers):
    ...
    assert api.put(KEYBACKUP_URL, {"blob": payload, "version": 3},
