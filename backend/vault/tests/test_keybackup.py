...
def test_put_then_get_round_trips_blob_and_version(api, active_user, device, auth_headers):
    ...
    assert api.put(KEYBACKUP_URL, {"blob": blob, "version": 3},
                   format="json", **headers).status_code == 200
...
def test_version_must_strictly_increase(api, active_user, device, auth_headers):
    headers = auth_headers(active_user, device)
    assert api.put(KEYBACKUP_URL, {"blob": backup_blob(b"A"), "version": 5},
                   format="json", **headers).status_code == 200

    equal = api.put(KEYBACKUP_URL, {"blob": backup_blob(b"B"), "version": 5},
                    format="json", **headers)
    assert equal.status_code == 409 and equal.json()["code"] == "stale_version"

    lower = api.put(KEYBACKUP_URL, {"blob": backup_blob(b"C"), "version": 4},
                    format="json", **headers)
    ...
    higher = api.put(KEYBACKUP_URL, {"blob": backup_blob(b"D"), "version": 6},
                     format="json", **headers)
    assert higher.status_code == 200
    assert api.get(KEYBACKUP_URL, **headers).json() == {
        "blob": backup_blob(b"D"), "version": 6}
