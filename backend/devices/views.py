...
# §A4: "cap 100 stored/device" for key packages. One-time prekeys get the same
# treatment at §A5's per-payload ceiling — without a stored cap, replenishment is an
# unbounded write primitive for any authenticated device.
...
def _device_list_etag(user_id):
    ...
    every add/remove/revoke (§A5), but it would also give an account whose devices
    were all revoked a different tag from an account that never had any — while
    both serve the same empty list. That difference is readable by any authenticated
    peer, so it leaks device churn the peer list deliberately hides (§A4).
    Dropping a device from the live set changes the hash just as well.

    `user__is_active` matches the peer view's filter: without it a deactivation
    would leave the tag unchanged and every polling peer would sit on a 304 holding
    devices the list no longer returns.
    """
...
class MyDevicesView(APIView):
    """`POST` registers a cryptographic device, `GET` lists this account's devices —
    both on §A5's `/me/devices`.

    A register-scope token's entire power is reaching the registration endpoint
    (§A8), so the scope check is per-method: POST opts down to `IsAuthenticated`,
    everything else keeps the project-wide `IsFullScope`. A register token must never
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            # (the emitted SQL carries no FOR UPDATE at all), and even a real row lock
            # over existing devices would not block a concurrent INSERT. Measured:
            # four concurrent registrations at a cap of 10 committed 12 devices.
            ...
            device = Device.objects.create(
                ...
                ik_pub=d["ik_raw"], spk_id=d["spk_id"], spk_pub=d["spk_raw"],
                spk_sig=d["spk_sig_raw"], registration_id=d["registration_id"],
                label_blob=d["label_raw"])  # spk_updated_date is auto_now_add
            if d["otpks"]:
                OneTimePrekey.objects.bulk_create([
                    OneTimePrekey(device=device, key_id=o["key_id"], pub=o["raw"])
                    for o in d["otpks"]])
            if d["kp_raws"]:
                KeyPackage.objects.bulk_create([
                    KeyPackage(device=device, blob=b) for b in d["kp_raws"]])
    ...
    def get(self, request):
        ...
        devices = Device.objects.filter(
            ...
            # created_date is day-coarse (§A4), so it alone leaves same-day devices in
            # an arbitrary order that can shuffle between polls; id breaks the tie.
            "id", "label_blob", "created_date", "last_active_date").order_by(
            "created_date", "id")
        data = [{
            "device_id": str(dv.id),
            "label_blob": base64.b64encode(bytes(dv.label_blob)).decode() if dv.label_blob else None,
            "created_date": dv.created_date.isoformat(),
            "last_active_date": dv.last_active_date.isoformat() if dv.last_active_date else None,
            "this_device": (dv.id == this_id),
        } for dv in devices]
...
class MyDeviceDetailView(APIView):
    ...
    def put(self, request, device_id):
        ...
        ).update(label_blob=ser.validated_data["label_raw"])
        if not updated:
    ...
    def delete(self, request, device_id):
        with transaction.atomic():
            ...
            device.token_generation += 1  # invalidate all its outstanding tokens (§A8)
    ...
    def _close_sockets(device_id):
        """Best-effort: tell the realtime consumer (if any) to drop this device's sockets.
        Safe no-op when no consumer/socket exists (§A6)."""
        try:
...
class _OwnDeviceView(APIView):
    """Shared gate for the four endpoints only the device itself may use: the token's
    device_id must equal the path device (§A5)."""
...
class MyPrekeysView(_OwnDeviceView):
    """Only the device itself may replenish its prekeys (token device_id must match)."""

    def put(self, request, device_id):
        denied = self._reject_other_device(request, device_id)
        if denied is not None:
            return denied
        ser = PrekeyReplenishSerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        d = ser.validated_data
        spk = d.get("spk")
        with transaction.atomic():
            ...
            if stored + len(d["otpks"]) > MAX_STORED_OTPKS:
            ...
            if d["otpks"]:
                ...
                OneTimePrekey.objects.bulk_create([
                    OneTimePrekey(device_id=device_id, key_id=o["key_id"], pub=o["raw"])
                    for o in d["otpks"]], ignore_conflicts=True)
...
class MyKeyPackagesView(_OwnDeviceView):
    def put(self, request, device_id):
        ...
        if denied is not None:
            return denied
        ser = KeyPackageUploadSerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        raws = ser.validated_data["kp_raws"]
        with transaction.atomic():
            Device.objects.select_for_update().filter(pk=device_id).only("id").first()
            ...
            if raws:
                KeyPackage.objects.bulk_create(
                    [KeyPackage(device_id=device_id, blob=b) for b in raws])
...
class PeerDevicesView(APIView):
    ...
    def get(self, request, user_id):
        ...
        data = [{"device_id": str(dv.id),
                 "ik_pub": base64.b64encode(bytes(dv.ik_pub)).decode(),
                 "registration_id": dv.registration_id} for dv in devices]
...
class _ClaimView(APIView):
    ...
    def _targets(request, user_id, fields):
        ...
        if "device_ids" in ser.validated_data:
            qs = qs.filter(id__in=ser.validated_data["device_ids"])
        return qs.only(*fields).order_by("id")


class ClaimKeysView(_ClaimView):
    """Return an X3DH bundle per requested live device, consuming one one-time prekey each
    (deleted transactionally, skip_locked so a claim can never hand the same OTPK twice)."""
