...
def otpks(count, start=0):
    return [{"key_id": start + i, "pub": pubkey(bytes([65 + (i % 26)]))}
            for i in range(count)]


def test_a_device_replenishes_its_own_prekeys(api, active_user, device, auth_headers):
    response = api.put(prekeys_url(device.id), {"otpks": otpks(5)}, format="json",
...
def test_re_uploading_a_key_id_is_an_idempotent_retry(api, active_user, device,
                                                      auth_headers):
...
def test_a_duplicate_key_id_within_one_payload_is_a_400(api, active_user, device,
                                                        auth_headers):
    body = {"otpks": [{"key_id": 4, "pub": pubkey(b"a")},
                      {"key_id": 4, "pub": pubkey(b"b")}]}
...
def test_a_signed_prekey_missing_its_signature_is_rejected(api, active_user, device,
                                                           auth_headers):
...
def test_replenishment_past_the_stored_cap_is_refused(api, active_user, device,
                                                      auth_headers):
    ...
    response = api.put(prekeys_url(device.id), {"otpks": otpks(1)}, format="json",
                       **auth_headers(active_user, device))
...
def test_a_refused_replenishment_rotates_nothing(api, active_user, device,
                                                 auth_headers):
    ...
    body = {"spk": {"spk_id": 4242, "pub": pubkey(b"n"), "sig": pubkey(b"z")},
            "otpks": otpks(1)}
...
def test_the_count_endpoint_reports_the_pool(api, active_user, device, auth_headers):
    ...
    response = api.get(f"{prekeys_url(device.id)}/count",
                       **auth_headers(active_user, device))
...
def test_a_device_cannot_touch_a_peers_prekeys(api, active_user, device, auth_headers,
                                               peer, peer_device):
    response = api.put(prekeys_url(peer_device.id), {"otpks": otpks(1)}, format="json",
                       **auth_headers(active_user, device))
...
def test_unknown_fields_are_rejected(api, active_user, device, auth_headers):
    response = api.put(prekeys_url(device.id), {"otpks": [], "private": "no"},
...
def test_an_empty_body_is_a_no_op(api, active_user, device, auth_headers):
    response = api.put(prekeys_url(device.id), {}, format="json",
                       **auth_headers(active_user, device))
