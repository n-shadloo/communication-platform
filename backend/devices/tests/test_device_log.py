...
def test_appends_assign_seq_and_read_back_byte_identical(api, active_user, device,
                                                         auth_headers, peer,
                                                         peer_device):
    mine = auth_headers(active_user, device)
    blobs = [log_blob(bytes([65 + i])) for i in range(3)]

    posted = api.post(LOG_URL, {"records": [{"blob": b} for b in blobs]},
                      format="json", **mine)

    assert posted.status_code == 201
    assert posted.json() == {"first_seq": 0, "last_seq": 2}
    page = api.get(peer_log_url(active_user.id),
...
def test_keyset_paging_clamps_bad_cursors_and_limits(api, active_user, device,
                                                     auth_headers, peer, peer_device):
    mine = auth_headers(active_user, device)
    api.post(LOG_URL, {"records": [{"blob": log_blob(bytes([65 + i]))}
                                   for i in range(5)]}, format="json", **mine)
    ...
    junk = api.get(peer_log_url(active_user.id, "?after=abc&limit=zzz"),
                   **theirs).json()
    assert [r["seq"] for r in junk["records"]] == [0, 1, 2, 3, 4]


def test_an_off_bucket_record_is_rejected_without_echoing(api, active_user, device,
                                                          auth_headers):
    off_bucket = base64.b64encode(b"x" * 300).decode()

    response = api.post(LOG_URL, {"records": [{"blob": off_bucket}]},
...
def test_more_than_fifty_records_per_append_are_rejected(api, active_user, device,
                                                         auth_headers):
    body = {"records": [{"blob": log_blob()} for _ in range(51)]}

    response = api.post(LOG_URL, body, format="json",
                        **auth_headers(active_user, device))

    assert response.status_code == 400


def test_a_log_append_changes_the_device_list_etag(api, active_user, device,
                                                   auth_headers):
    ...
    assert api.get(DEVICES_URL, HTTP_IF_NONE_MATCH=after["ETag"],
                   **headers).status_code == 304


def test_the_peer_list_surfaces_the_log_head(api, active_user, device, auth_headers,
                                             peer, peer_device):
    api.post(LOG_URL, {"records": [{"blob": log_blob()}, {"blob": log_blob(b"S")}]},
             format="json", **auth_headers(peer, peer_device))

    body = api.get(f"/api/v1/users/{peer.id}/devices",
                   **auth_headers(active_user, device)).json()

    assert body["log_head_seq"] == 1


def test_a_deactivated_users_log_is_hidden(api, active_user, device, auth_headers,
                                           peer, peer_device):
    api.post(LOG_URL, {"records": [{"blob": log_blob()}]}, format="json",
             **auth_headers(peer, peer_device))
...
def test_a_non_linking_record_is_accepted_and_served_unchanged(api, active_user,
                                                               device, auth_headers,
                                                               peer, peer_device):
    ...
    posted = api.post(LOG_URL, {"records": [{"blob": garbage}]}, format="json",
                      **mine)
...
class DeviceLogAppendConcurrencyTests(TransactionTestCase):
    ...
    def test_parallel_appends_never_duplicate_or_gap_seq(self):
        ...
        threads = [threading.Thread(target=append_one)
                   for _ in range(CONCURRENT_APPENDS)]
        ...
        seqs = sorted(DeviceLogRecord.objects.filter(user_id=self.owner.id)
