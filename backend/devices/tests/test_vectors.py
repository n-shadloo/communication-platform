...
VECTORS_PATH = (pathlib.Path(__file__).resolve().parents[1] / "vectors"
                / "vectors.json")
...
def test_every_vector_signature_verifies_against_its_stated_public_key(name):
    ...
    pub.verify(bytes.fromhex(vector["signature_hex"]),
               bytes.fromhex(vector["signed_bytes_hex"]))
...
def test_absent_pq_fields_are_zero_length_not_skipped():
    ...
    assert vector["signed_bytes_hex"] != VECTORS[
        "device_bundle_hybrid"]["signed_bytes_hex"]
...
@pytest.mark.parametrize("name,domain,id_key,pub_key", [
    ("spk_sig", "chat:v1:signed-prekey", "spk_id", "spk_pub_hex"),
    ("pq_spk_sig", "chat:v1:pq-signed-prekey", "pq_spk_id", "pq_spk_pub_hex"),
])
def test_the_prekey_signatures_match_their_documented_encodings(name, domain, id_key,
                                                                pub_key):
    vector = VECTORS[name]
    fields = vector["fields"]

    expected = encode(domain, uuid.UUID(fields["user_id"]).bytes,
                      u32(fields[id_key]), hexed(vector, pub_key))

    assert expected.hex() == vector["signed_bytes_hex"]


def test_the_two_prekey_domains_differ_so_neither_signature_replays_as_the_other():
    assert (VECTORS["spk_sig"]["signed_bytes_hex"]
