...
def test_uploads_accumulate(api, active_user, device, auth_headers):
    ...
    api.put(
        keypackages_url(device.id), {"keypackages": blobs(2)}, format="json", **headers
    )

    response = api.put(
        keypackages_url(device.id), {"keypackages": blobs(3)}, format="json", **headers
    )
...
def test_a_batch_that_would_cross_the_cap_is_refused_whole(
...
def test_the_count_endpoint_reports_the_store(api, active_user, device, auth_headers):
    ...
    response = api.get(
        f"{keypackages_url(device.id)}/count", **auth_headers(active_user, device)
    )
...
def test_more_than_a_hundred_in_one_request_is_rejected(
    api, active_user, device, auth_headers
):
    response = api.put(
        keypackages_url(device.id),
        {"keypackages": blobs(101)},
        format="json",
        **auth_headers(active_user, device),
    )
...
def claim_keypackages(api, headers, user_id):
    return api.post(
        f"/api/v1/users/{user_id}/keypackages/claim", {}, format="json", **headers
    ).json()["keypackages"]


def test_a_non_last_resort_package_is_preferred_and_deleted(
    api, active_user, device, auth_headers, peer, peer_device
):
    last = KeyPackage.objects.create(
        device=peer_device, blob=b"L" * 4096, is_last_resort=True
    )
...
def test_the_last_resort_package_is_served_but_never_deleted(
    api, active_user, device, auth_headers, peer, peer_device
):
    ...
    KeyPackage.objects.create(device=peer_device, blob=b"L" * 4096, is_last_resort=True)
    headers = auth_headers(active_user, device)
    ...
    assert KeyPackage.objects.filter(device=peer_device, is_last_resort=True).count() == 1


def test_an_exhausted_device_with_no_last_resort_yields_nothing(
    api, active_user, device, auth_headers, peer, peer_device
):
    assert claim_keypackages(api, auth_headers(active_user, device), peer.id) == []


def test_uploading_a_last_resort_package_replaces_the_previous_one(
    api, active_user, device, auth_headers
):
    headers = auth_headers(active_user, device)
    api.put(
    ...
    response = api.put(
        keypackages_url(device.id),
        {"keypackages": [keypackage_blob(b"2")], "is_last_resort": True},
        format="json",
        **headers,
    )
...
def test_the_count_reports_only_the_consumable_pool(
    api, active_user, device, auth_headers
):
