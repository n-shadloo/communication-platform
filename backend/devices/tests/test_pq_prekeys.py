...
def test_pq_material_round_trips_byte_identically(
    ...
    registered = api.post(
        DEVICES_URL, payload, format="json", **auth_headers(peer, peer_device)
    ).json()
...
def test_a_wrong_length_pq_key_is_a_400_not_a_500(
    api, active_user, device, auth_headers, nbytes
):
...
def test_a_wrong_length_pq_spk_is_a_400_not_a_500(api, active_user, device, auth_headers):
...
class PqClaimRaceTests(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.claimant = User.objects.create_user(
            username="claimant", password=PASSWORD, is_active=True
        )
        self.claimant_device = make_device(self.claimant, registration_id=1)
        self.owner = User.objects.create_user(
            username="owner", password=PASSWORD, is_active=True
        )
    ...
    def test_one_pq_prekey_is_handed_to_at_most_one_concurrent_claimant(self):
        ...
        self.assertEqual(
            received, [77], f"the single PQ OTPK went to {len(received)} claimants"
        )
        self.assertEqual(
