...
from .conftest import PASSWORD, connect_then_wait, make_device, stock_prekeys
...
class ClaimRaceTests(TransactionTestCase):
    ...
    def test_one_prekey_is_handed_to_at_most_one_concurrent_claimant(self):
        ...
        received = self._storm(f"/api/v1/users/{self.owner.id}/keys/claim", self._otpk_of)

        self.assertEqual(
            received, [99], f"the single OTPK went to {len(received)} claimants"
        )
        self.assertEqual(
