...
def test_a_bundle_carries_the_public_x3dh_material(api, active_user, device,
...
def test_claiming_consumes_the_one_time_prekey(api, active_user, device, auth_headers,
                                               peer, peer_device):
...
def test_each_claim_takes_a_different_prekey(api, active_user, device, auth_headers,
                                             peer, peer_device):
    stock_prekeys(peer_device, 3)
    headers = auth_headers(active_user, device)

    seen = [api.post(claim_url(peer.id), {}, format="json",
                     **headers).json()["bundles"][0]["otpk"]["key_id"]
            for _ in range(3)]
...
def test_device_ids_narrows_the_claim(api, active_user, device, auth_headers, peer,
                                      peer_device):
    other = make_device(peer, registration_id=556)

    response = api.post(claim_url(peer.id), {"device_ids": [str(other.id)]},
...
def test_an_explicit_empty_device_id_list_claims_nothing(api, active_user, device,
                                                         auth_headers, peer,
                                                         peer_device):
    """Treating `[]` as "all" would silently burn a one-time prekey on every device."""
    stock_prekeys(peer_device, 1)

    response = api.post(claim_url(peer.id), {"device_ids": []}, format="json",
                        **auth_headers(active_user, device))
...
def test_a_malformed_device_id_is_a_400_not_a_500(api, active_user, device,
                                                  auth_headers, peer, peer_device):
    """An unparsed value reaching a uuid column raises Django's ValidationError, which
    DRF does not handle."""
    response = api.post(claim_url(peer.id), {"device_ids": ["not-a-uuid"]},
                        format="json", **auth_headers(active_user, device))

    assert response.status_code == 400


def test_a_json_array_body_is_a_400_not_a_500(api, active_user, device, auth_headers,
                                              peer):
    response = api.post(claim_url(peer.id), ["nope"], format="json",
                        **auth_headers(active_user, device))

    assert response.status_code == 400


def test_revoked_devices_are_not_claimable(api, active_user, device, auth_headers,
                                           peer, peer_device):
    stock_prekeys(peer_device, 1)
...
def test_a_deactivated_users_devices_are_not_claimable(api, active_user, device,
                                                       auth_headers, peer, peer_device):
...
def test_key_packages_are_handed_out_once_each(api, active_user, device, auth_headers,
                                               peer, peer_device):
...
def test_claiming_a_users_own_devices_is_normal(api, active_user, device, auth_headers):
    ...
    response = api.post(claim_url(active_user.id), {"device_ids": [str(mine.id)]},
                        format="json", **auth_headers(active_user, device))
