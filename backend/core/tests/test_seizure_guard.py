...
FORBIDDEN_FIELD_NAMES = frozenset({
    ...
    "media_key", "sframe_key", "room_key",
    "sender", "sender_id", "recipient_id", "recipients",
    "members", "membership", "roster", "group_members",
    "password_plain",
})
...
def envelope_graph_offenders():
    ...
    for field in _concrete_fields(QueuedEnvelope):
        ...
        if field.is_relation:
            fks.append(field)
    if [f"{fk.related_model._meta.app_label}.{fk.related_model.__name__}"
            for fk in fks] != ["devices.Device"]:
        offenders.append(
...
def dual_user_fk_offenders():
    ...
    for model in apps.get_models():
        if model._meta.app_label == "accounts":
            continue
        user_fks = [f.name for f in _concrete_fields(model)
...
class SeizureGuardTests(SimpleTestCase):
    ...
    def test_the_guard_actually_sees_this_schema(self):
        ...
        self.assertLessEqual(
            ...
            seen)
        blob_names = {
            f"{_label(m)}.{f.name}" for m in apps.get_models()
