...
from core.tests.test_seizure_guard import (dual_user_fk_offenders,
...
def _second_device(user, registration_id):
    return Device.objects.create(user=user, ik_pub=b"ik2", spk_id=2, spk_pub=b"spk2",
                                 spk_sig=b"sig2", registration_id=registration_id)
...
def test_revoking_a_device_cuts_its_access_and_destroys_its_state(api, active_user,
    ...
    QueuedEnvelope.objects.create(recipient_device=doomed, seq=1,
                                  blob=b"\xa5" * min(ENVELOPE_BUCKETS))
    doomed_headers = {"HTTP_AUTHORIZATION": f"Bearer {doomed_access}"}
    assert api.get(DEVICES_URL, **doomed_headers).status_code == 200

    assert api.delete(f"{DEVICES_URL}/{doomed.id}",
                      **auth_headers(active_user, device)).status_code == 204
...
def test_fanout_writes_independent_copies_and_never_a_sender(api, active_user, device,
    ...
    bob = User.objects.create_user(username="bob", password="x-trellis-9-quartz",
                                   is_active=True)
    targets = [_second_device(bob, 9003), _second_device(bob, 9004)]
    blob = _b64(min(ENVELOPE_BUCKETS), 0x5A)

    resp = api.post(ENVELOPES_URL,
                    {"messages": [{"device_id": str(d.id), "blob": blob}
                                  for d in targets]},
    ...
    for row in rows:
        stored = {str(value) for value in
                  (row.id, row.recipient_device_id, row.seq, row.queued_hour)}
