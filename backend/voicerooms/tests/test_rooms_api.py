...
def test_an_unknown_field_is_rejected(api, active_user, device, auth_headers):
    resp = api.post(
        "/api/v1/rooms",
        {"name_blob": name_blob_b64(b"q"), "junk": 1},
        format="json",
        **auth_headers(active_user, device),
    )
...
def test_get_round_trips_the_blob_and_reports_zero_live(
    api, active_user, device, auth_headers, room
):
...
def test_unknown_room_is_404_for_get_put_and_token(
    api, active_user, device, auth_headers
):
    ...
    assert (
        api.put(
            f"/api/v1/rooms/{missing}",
            {"name_blob": name_blob_b64()},
            format="json",
            **headers,
        ).status_code
        == 404
...
def test_off_bucket_names_are_rejected_without_echo(
    api, active_user, device, auth_headers, room, bad
):
    for verb, url in (("post", "/api/v1/rooms"), ("put", f"/api/v1/rooms/{room.id}")):
        resp = getattr(api, verb)(
            url, {"name_blob": bad}, format="json", **auth_headers(active_user, device)
        )
        assert resp.status_code == 400
...
def test_missing_name_blob_is_a_400(api, active_user, device, auth_headers):
    resp = api.post(
        "/api/v1/rooms", {}, format="json", **auth_headers(active_user, device)
    )
    assert resp.status_code == 400
...
def test_register_scope_tokens_are_locked_out_of_every_room_endpoint(
    ...
    assert (
        api.post(
            "/api/v1/rooms", {"name_blob": name_blob_b64()}, format="json", **headers
        ).status_code
        == 403
    )
...
def test_anonymous_requests_are_401(api, room):
    assert (
        api.post(
            "/api/v1/rooms", {"name_blob": name_blob_b64()}, format="json"
        ).status_code
        == 401
    )
